...
def expand(prompt):
    ...
    re_prompt = '''
        ...
        正向提示词
        1. 设定场景：根据商品的特点和目标受众，选择合适的场景;
        2、营造氛围：描述光线、色彩、天气等元素来营造氛围;
        3、图片风格：可以是写实、卡通、复古、现代简约等风格;
        4、画质品质：使用高分辨率、清晰细节、鲜艳色彩等词汇来提升画质;
    ''' %(prompt)
...
def expand_tag(prompt, tags):
    ...
    # 3、规避不适当内容：确保生成的图片不包含不适当或不符合品牌形象的内容，如不含成人内容。
    re_prompt = '''
        撰写图生图的提示词，商品图片以及商品信息已经给出，商品信息：%s ,请按如下模版输出提示词，每个模版给出标签提示，按标签内容生产提示词，重要，重要，重要，最后输出的只有英文提示词，模版结构如下
        正向提示词
        1. 设定场景：根据商品的特点和目标受众，选择合适的场景:%s;
        2、营造氛围：描述光线、色彩、天气等元素来营造氛围:%s;
        3、图片风格：可以是写实、卡通、复古、现代简约等风格:%s;
        4、画质品质：使用高分辨率、清晰细节、鲜艳色彩等词汇来提升画质:%s;
    ''' %(prompt, tags[0], tags[1], tags[2], tags[3])
